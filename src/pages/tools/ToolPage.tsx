...
import { ArrowLeft, CheckCircle2, Download, Loader2, RotateCcw, AlertCircle, ChevronRight } from "lucide-react";
...
import {
  ...
  downloadBlob, formatBytes, ToolResult,
} from "@/lib/pdf";
...
const ToolPage = () => {
  ...
  const reset = () => {
    setFiles([]); setResult(null); setError(null); setState("idle"); setProgress(0);
  };

  const run = async () => {
    setError(null); setResult(null); setState("uploading"); setProgress(15);
    try {
      // simulate quick upload step for UX feedback
      await new Promise((r) => setTimeout(r, 350));
      setState("processing"); setProgress(55);
